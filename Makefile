...
.PHONY: build_config
build_config: ### Will start configurator
	go run ./cmd/main.go

.PHONY: build_and_run_config
build_and_run_config: ### Will start configurator and after will run created config
	go run ./cmd/main.go && docker compose -f docker-compose.yaml watch
